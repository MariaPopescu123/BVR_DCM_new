...
#read in CSVs (all keyed on Date). Strip columns from right-hand frames that
#also appear on the left-hand spine (Year, WaterLevel_m, temp_at_DCM) so the
#left_joins below don't create .x/.y collisions.
...
final_photic_thermo <- read_csv("CSVs/final_photic_thermo.csv") |>
  mutate(Date = as_date(Date)) |>
  select(-any_of(c("WaterLevel_m", "temp_at_DCM")))
