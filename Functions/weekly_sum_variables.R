#Summarizes variables by (Year, Week): max/min values, depths of extremes, and value at DCM.
#Example: weekly_sum_variables(metals_df, c("SFe_mgL", "TFe_mgL"))
...
weekly_sum_variables <- function(df, variables) {
  ...
  #only keep weeks with >4m depth range (enough profile coverage)
  ...
  .value_at_dcm <- function(.df, var_name) {
    ...
    #if DCM deeper than deepest sample, use deepest available
    if (dcm >= max_depth) {
