...
var_importance_shap_plots <- function(Xdataframe, XYear, XYear2, whichvars, response_var, save_dir){
  ...
  this <- ggplot(filtered_importance_df, aes(x = `%IncMSE`, y = reorder(Variable, `%IncMSE`))) +
    ...
    labs(
      title = paste0(save_dir,"Variable Importance based on % IncMSE ", XYear, "-", XYear2," ", whichvars),
      x = "% IncMSE",
      y = "Variables"
    ) +
    theme_minimal()
  ...
  p <- df %>%
    ...
    labs(
      title = paste(save_dir, 'Distribution of SHAP values', XYear, "-", XYear2," ",whichvars),
      y = '',
